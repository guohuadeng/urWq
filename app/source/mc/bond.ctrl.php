...
if ($do == 'credits') {
	...
	if ($_GPC ['credittype']) {
		if ($_GPC ['type'] == 'order') {
			...
			$orderpager = pagination ( $total, $pindex, $psize, '', array (
					'before' => 0,
					'after' => 0,
					'ajaxcallback' => '' 
			) );
...
if ($do == 'address') {
	if (checksubmit ( 'submit' )) {
		...
		if (pdo_update ( 'mc_member_address', $address, array (
				'id' => intval ( $_GPC ['addid'] ),
				'uid' => $_W ['fans'] ['uid'] 
		) )) {
...
if ($do == 'card') {
	$mcard = pdo_fetch ( 'SELECT * FROM ' . tablename ( 'mc_card_members' ) . ' WHERE uniacid = :uniacid AND uid = :uid', array (
			':uniacid' => $_W ['uniacid'],
			':uid' => $_W ['member'] ['uid'] 
	) );
	if (! empty ( $mcard )) {
	...
	if (! empty ( $setting )) {
		...
		if ($coupon_id > 0) {
			$coupon = pdo_fetch ( 'SELECT couponid,title,type FROM ' . tablename ( 'activity_coupon' ) . ' WHERE uniacid = :uniacid AND couponid = :couponid', array (
					':uniacid' => $_W ['uniacid'],
					':couponid' => $coupon_id 
			) );
		}
	...
	if (checksubmit ( 'submit' )) {
		...
		$count = pdo_fetchcolumn ( $sql, array (
				':uid' => $_W ['member'] ['uid'],
				':cid' => $_GPC ['cardid'],
				':uniacid' => $_W ['uniacid'] 
		) );
...
if ($do == 'mycard') {
	...
	if (empty ( $mcard ['openid'] ) && ! empty ( $_W ['openid'] )) {
		pdo_update ( 'mc_card_members', array (
				'openid' => $_W ['openid'] 
		), array (
				'uniacid' => $_W ['uniacid'],
				'uid' => $_W ['member'] ['uid'] 
	...
	if (! empty ( $mcard ['status'] )) {
		$setting = pdo_fetch ( 'SELECT * FROM ' . tablename ( 'mc_card' ) . ' WHERE uniacid = :uniacid', array (
				':uniacid' => $_W ['uniacid'] 
		) );
		if (! empty ( $setting )) {
			// $setting['description']=html_entity_decode($setting['description'], ENT_QUOTES);
			$setting ['color'] = iunserializer ( $setting ['color'] );
			$setting ['background'] = iunserializer ( $setting ['background'] );
			...
			$unisetting = uni_setting ( $_W ['uniacid'], array (
					'recharge' 
			) );
...
if ($do == 'consume') {
	...
	$stores = pdo_fetchall ( 'SELECT id,business_name FROM ' . tablename ( 'activity_stores' ) . ' WHERE uniacid = :uniacid', array (
			':uniacid' => $_W ['uniacid'] 
	), 'id' );
	if (checksubmit ()) {
	...
	$url = $_W ['siteroot'] . 'app' . ltrim ( murl ( 'clerk/card', array (
			'uid' => $_W ['member'] ['uid'] 
	) ), '.' );
	template ( 'mc/consume' );
	exit ();
...
if ($do == 'mobile') {
	$profile = mc_fetch ( $_W ['member'] ['uid'], array (
			'mobile' 
	) );
	...
	if (checksubmit ( 'submit' )) {
		if ($mobile_exist == 1) {
			...
			$info = pdo_fetch ( 'SELECT uid, password, salt FROM ' . tablename ( 'mc_members' ) . ' WHERE uniacid = :uniacid AND mobile = :mobile AND uid = :uid', array (
					':uniacid' => $_W ['uniacid'],
					':mobile' => $oldmobile,
					':uid' => $_W ['member'] ['uid'] 
			) );
...
/* 				if($info['password'] == md5($password . $info['salt'] . $_W['config']['setting']['authkey'])) { 
			...
			$is_exist = pdo_fetch ( 'SELECT uid FROM ' . tablename ( 'mc_members' ) . ' WHERE uniacid = :uniacid AND mobile = :mobile AND uid != :uid', array (
					':uniacid' => $_W ['uniacid'],
					':mobile' => $mobile,
					':uid' => $_W ['member'] ['uid'] 
			) );
	...
	if (checksubmit ( 'submit' )) {
		...
		if ($type == 1) {
			if ($reregister) {
				...
				$emailexists = pdo_fetch ( "SELECT uid FROM " . tablename ( 'mc_members' ) . " WHERE email = :email AND uniacid = :uniacid AND uid != :uid ", array (
						':email' => $data ['email'],
						':uniacid' => $_W ['uniacid'],
						':uid' => $_W ['member'] ['uid'] 
				) );
			...
			if (! empty ( $member_old ) && ! empty ( $member_new )) {
				...
				pdo_update ( 'mc_members', $profile_update, array (
						'uid' => $member ['uid'],
						'uniacid' => $_W ['uniacid'] 
				) );
				pdo_delete ( 'mc_members', array (
						'uid' => $_W ['member'] ['uid'],
						'uniacid' => $_W ['uniacid'] 
				) );
				...
				pdo_update ( 'activity_exchange_trades', array (
						'uid' => $member ['uid'] 
				), array (
						'uid' => $_W ['member'] ['uid'],
						'uniacid' => $_W ['uniacid'] 
				) );
				pdo_update ( 'activity_exchange_trades_shipping', array (
						'uid' => $member ['uid'] 
				), array (
						'uid' => $_W ['member'] ['uid'],
						'uniacid' => $_W ['uniacid'] 
				) );
				pdo_update ( 'mc_credits_record', array (
						'uid' => $member ['uid'] 
				), array (
						'uid' => $_W ['member'] ['uid'],
						'uniacid' => $_W ['uniacid'] 
