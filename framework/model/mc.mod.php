...
function mc_check($data) {
	...
	if (! empty ( $data ['email'] )) {
		...
		$isexist = pdo_fetchcolumn ( 'SELECT COUNT(*) FROM ' . tablename ( 'mc_members' ) . ' WHERE uniacid = :uniacid AND email = :email AND uid != :uid', array (
				':uniacid' => $_W ['uniacid'],
				':email' => $email,
				':uid' => $_W ['member'] ['uid'] 
		) );
	...
	if (! empty ( $data ['mobile'] )) {
		...
		$isexist = pdo_fetchcolumn ( 'SELECT COUNT(*) FROM ' . tablename ( 'mc_members' ) . ' WHERE uniacid = :uniacid AND mobile = :mobile AND uid != :uid', array (
				':uniacid' => $_W ['uniacid'],
				':mobile' => $mobile,
				':uid' => $_W ['member'] ['uid'] 
		) );
...
function mc_update($uid, $fields) {
	...
	$isexists = pdo_fetchcolumn ( "SELECT uid FROM " . tablename ( 'mc_members' ) . " WHERE uid = :uid", array (
			':uid' => $uid 
	) );
	$condition = '';
	if (! empty ( $isexists )) {
	...
	if (! empty ( $fields ['email'] )) {
		$emailexists = pdo_fetchcolumn ( "SELECT email FROM " . tablename ( 'mc_members' ) . " WHERE uniacid = :uniacid AND email = :email " . $condition, array (
				':uniacid' => $_W ['uniacid'],
	...
	if (! empty ( $fields ['mobile'] )) {
		$mobilexists = pdo_fetchcolumn ( "SELECT mobile FROM " . tablename ( 'mc_members' ) . " WHERE uniacid = :uniacid AND mobile = :mobile " . $condition, array (
				':uniacid' => $_W ['uniacid'],
				':mobile' => trim ( $fields ['mobile'] ) 
		) );
		if ($mobilexists) {
			unset ( $fields ['mobile'] );
		}
	...
	if (empty ( $isexists )) {
		...
		if (is_string ( $uid_temp )) {
			pdo_update ( 'mc_mapping_fans', array (
					...
					'uniacid' => $_W ['uniacid'],
					'openid' => trim ( $uid_temp ) 
			) );
		}
		return $insert_id;
	} else {
		$result = pdo_update ( 'mc_members', $fields, array (
				'uid' => $uid 
		) );
		return $result > 0;
	}
...
function mc_fetch($uid, $fields = array()) {
	...
	if (is_array ( $uid )) {
		$result = pdo_fetchall ( "SELECT $select FROM " . tablename ( 'mc_members' ) . " WHERE uid IN ('" . implode ( "','", is_array ( $uid ) ? $uid : array (
				$uid 
		) ) . "')", array (), 'uid' );
		...
		$result = pdo_fetch ( "SELECT $select FROM " . tablename ( 'mc_members' ) . " WHERE `uid` = :uid", array (
				':uid' => $uid 
		) );
	...
	if (! empty ( $_SESSION ['openid'] ) && intval ( $_W ['account'] ['level'] ) >= 3) {
		...
		if (! is_error ( $userinfo ) && ! empty ( $userinfo ) && is_array ( $userinfo ) && ! empty ( $userinfo ['nickname'] )) {
			...
			if (! empty ( $fan ['uid'] ) || ! empty ( $_SESSION ['uid'] )) {
				...
				if (! empty ( $record )) {
					pdo_update ( 'mc_members', $record, array (
							'uid' => intval ( $uid ) 
					) );
				}
			}
...
function mc_require($uid, $fields, $pre = '') {
	...
	$sql = 'SELECT `f`.`field`, `f`.`id` AS `fid`, `mf`.* FROM ' . tablename ( 'profile_fields' ) . " AS `f` LEFT JOIN " . tablename ( 'mc_member_fields' ) . " AS `mf` ON `f`.`id` = `mf`.`fieldid` WHERE `uniacid` = :uniacid ORDER BY
			`displayorder` DESC";
	$system_fields = pdo_fetchall ( $sql, array (
			':uniacid' => $_W ['uniacid'] 
	), 'field' );
	...
	if (! empty ( $message )) {
		...
		if (checksubmit ( 'submit' )) {
			...
			if (empty ( $uid )) {
				pdo_update ( 'mc_oauth_fans', array (
						'uid' => $insertuid 
				), array (
						'oauth_openid' => $_W ['openid'] 
				) );
				pdo_update ( 'mc_mapping_fans', array (
						'uid' => $insertuid 
				), array (
...
function mc_credit_fetch($uid, $types = array()) {
	...
	return pdo_fetch ( "SELECT {$select} FROM " . tablename ( 'mc_members' ) . ' WHERE uid = :uid LIMIT 1', array (
			':uid' => $uid 
	) );
}
...
function mc_fields() {
	...
	$memberFields = pdo_fetchall ( $sql, array (
			':uniacid' => $_W ['uniacid'] 
	), 'fieldid' );
...
function mc_handsel($touid, $fromuid, $handsel, $uniacid = '') {
	...
	$touid_exist = mc_fetch ( $touid, array (
			'uniacid' 
	) );
	...
	$creditbehaviors = pdo_fetchcolumn ( 'SELECT creditbehaviors FROM ' . tablename ( 'uni_settings' ) . ' WHERE uniacid = :uniacid', array (
			':uniacid' => $uniacid 
	) );
...
function mc_group_update() {
	...
	if (! $credit) {
		$credit = pdo_fetchcolumn ( 'SELECT credit1 FROM ' . tablename ( 'mc_members' ) . ' WHERE uniacid = :uniacid AND uid = :uid', array (
				':uniacid' => $_W ['uniacid'],
				':uid' => $_W ['member'] ['uid'] 
		) );
	}
